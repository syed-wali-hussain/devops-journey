...
# 📱 LINKEDIN POST (Portfolio Building)

```
[Headline]
🔐 Day 1 of my DevOps journey: SSH Keys, Server Hygiene & Production Fundamentals

[Post]
Just completed Week 1 of my 36-week DevOps bootcamp! Here's what I learned:

✅ Generated SSH key pairs (RSA-4096) for secure server access
✅ Connected to Azure VM with key-based authentication (no passwords!)
✅ Created service users with sudo access and proper permissions
✅ Debugged 3 real-world SSH incidents:
   • Port change causing connection timeout (port 22 → 2222)
   • Permission denied due to wrong file permissions (600 vs 666)
   • Key mismatch between users (devops-user vs azureuser)

🔑 Key Takeaways:
- Security > Convenience (always use SSH keys, not passwords)
- Permissions matter! 600 for private keys, 700 for directories
- Documentation is critical (updated SSH config & incident reports)
- Troubleshooting methodology: Verbose output → Root cause → Fix → Document

💡 Beyond the syllabus:
- Explored SSH agent for key management
- Created ~/.ssh/config for convenient aliases
- Learned about bastion hosts for secure prod access
- Implemented SSH hardening rules (sshd_config)

This week taught me that DevOps isn't just about big tools—it's about mastering fundamentals and debugging like a pro. Every incident is a learning opportunity.

Next: Git + Version Control fundamentals

#DevOps #Linux #SSH #SecurityFirst #CloudEngineering #LearningJourney

[Engagement Hook]
"What's your biggest SSH security lesson? Drop it in the comments 👇"

[Hashtags]
#DevOpsEngineer #Linux #CloudSecurity #AzureCloud #CI/CD #DevOpsBootcamp #SysAdmin #Automation
```
...
```markdown
# DevOps Week 1: SSH & Linux Fundamentals

**Repository:** devops-learning-week1  
**Status:** Complete ✅  
**Last Updated:** December 19, 2024

## Overview

Complete hands-on learning guide for PHASE 1, WEEK 1 of the DevOps bootcamp.
...
```bash
# You need:
- A Linux/Mac terminal (or WSL on Windows)
- An Azure VM (Ubuntu 22.04) or any Linux server
- ~2 hours for complete walkthrough
```

### Lab 1: SSH Key Setup (30 minutes)
...
### Lab 2: User & Sudo Setup (30 minutes)
...
id devops-user  # Should show group=27(sudo)
```
...
├── README.md                          (This file)
├── docs/
│   ├── 01-concepts.md                 (Detailed theory)
│   ├── 02-ssh-keys-explained.md       (SSH deep dive)
│   ├── 03-user-management.md          (Users & permissions)
│   └── 04-server-hardening.md         (Security best practices)
├── labs/
│   ├── lab-1-ssh-keys/
│   │   ├── README.md                  (Lab instructions)
│   │   ├── verify.sh                  (Verification script)
│   │   └── solution/                  (Solution examples)
│   └── lab-2-users-sudo/
│       ├── README.md
│       ├── verify.sh
│       └── solution/
├── incidents/
│   ├── incident-1-wrong-port.md       (Port change issue)
│   ├── incident-2-permissions.md      (Permission errors)
│   ├── incident-3-key-mismatch.md     (Wrong key/user)
│   └── resolution-flowchart.md        (Debugging guide)
├── configs/
│   ├── ssh-config.example             (~/.ssh/config template)
│   ├── sshd-config.hardened           (/etc/ssh/sshd_config)
│   └── bashrc-ssh-setup.sh            (Auto SSH agent setup)
├── scripts/
│   ├── setup-dev-env.sh               (Automated setup)
│   ├── create-users.sh                (User creation script)
│   ├── check-permissions.sh           (Security audit)
│   └── monitoring-ssh.sh              (Log monitoring)
└── progress-checklist.md              (Verification tasks)
```
...
- SSH config aliases and ProxyJump
...
- [Linux File Permissions](https://www.gnu.org/software/coreutils/manual/html_node/chmod-invocation.html)
...
**Q: Should I use password authentication?**
A: Never in production. SSH keys only. Passwords are for initial setup only.

**Q: How often rotate SSH keys?**
A: Every 3-6 months, or immediately if compromised.

**Q: What if I lose my private key?**
A: You'll need physical access to server to re-add new key. Always backup!

**Q: Ed25519 vs RSA-4096?**
A: Ed25519 is more secure, smaller, faster. Use Ed25519 if available.

**Q: Can I use one key for multiple servers?**
A: Yes, but recommended to use different keys per server for security.
...
- [ ] Complete all labs and incidents
- [ ] Create incident documentation
- [ ] Post on LinkedIn
- [ ] Review advanced topics
- [ ] Start Week 2: Git + Version Control
```
...
## Summary Checklist - WEEK 1 COMPLETE
